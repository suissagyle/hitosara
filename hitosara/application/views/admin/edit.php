...
<body>
	...
	<div class="col-sm-9 col-sm-offset-3 col-lg-10 col-lg-offset-2 main">
		...
		<div class="row">
			<div class="col-lg-12">
				<h1 class="page-header">アプリケーションを編集</h1>
			</div>
		</div><!--/.row-->
		...
		<div class="panel panel-default">
			...
			<div class="panel-heading">各種パラメータ</div>
			<div class="panel-body">
				<?php 
					echo form_open_multipart('index.php/Application/edit/' . html_escape($apply->id) . ''); 
					echo form_hidden('id', html_escape($apply->id));
				?>
					<div class="form-group">
						<label>お支払いフラグ</label>
						<select class="form-control" name="status">
							<option <?php if ($apply->status=="新しい契約") echo "selected";?> value="新しい契約">新しい契約</option>
							<option <?php if ($apply->status=="審査中") echo "selected";?> value="審査中">審査中</option>
							<option <?php if ($apply->status=="支払手続き中") echo "selected";?> value="支払手続き中">支払手続き中</option>
							<option <?php if ($apply->status=="サービス開始済み") echo "selected";?> value="サービス開始済み">契約、サービス開始済み</option>
						</select>
					</div>
					<table class="table table-striped">
				    	<tr>
				      		<th scope="row">申込登録日</th>
				      		<td><?php echo html_escape($apply->date_added); ?></td>
				    	</tr>
						<tr>
				      		<th scope="row">更新日</th>
				      		<td><?php echo html_escape($apply->date_updated); ?></td>
				    	</tr>
					</table>
					<div class="form-group">
						<label>備考</label>
						<textarea name="remarks" class="form-control" rows="3"><?php echo html_escape($apply->remarks); ?></textarea>
					</div>
					<?php
					    //新規登録ボタン（下部）
					    echo form_submit("update", "更新", "class='btn btn-primary'");
					?>
				<?php echo form_close(); ?>
			</div>
